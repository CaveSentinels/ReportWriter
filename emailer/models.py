...
# Check with Enhanced CWE Application
# This method will send an email when the MUO is commented upon
@receiver(report_commented_on)
def on_report_commented(sender, comment, request, **kwargs):
...
def on_report_submitted_for_review(sender,instance, **kwargs):
    muo_container_type = ContentType.objects.get(app_label='report', model='report')
    ...
    users = User.objects.filter(profile__notify_report_submitted_review=True)\
